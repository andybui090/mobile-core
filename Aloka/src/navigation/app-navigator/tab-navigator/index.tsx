...
function NearbyStack() {
  return (
    <StackNearby.Navigator
      screenOptions={screenOptions}
      initialRouteName={nearbyTabRoute.nearbyScreen}>
      {Object.values(nearbyTabRoute).map(item => {
        return (
          <StackNearby.Screen
            key={item}
            name={item}
            component={nearbyStack[item as keyof typeof nearbyStack]}
          />
        );
      })}
    </StackNearby.Navigator>
...
function CourseStack() {
  return (
    <StackDiscover.Navigator
      screenOptions={screenOptions}
      initialRouteName={courseTabRouteV2.coursesScreenV2}>
      {Object.values(courseTabRouteV2).map(item => {
        return (
          <StackDiscover.Screen
            key={item}
            name={item}
            component={courseStackV2[item as keyof typeof courseStackV2]}
          />
        );
      })}
    </StackDiscover.Navigator>
