...
const VideosLiked: React.FC<any> = () => {
  ...
  const [listVideoLiked, setListVideoLiked] = useState<any[]>([]);
  const [offset, setOffset] = useState<number>(0);
  const [finalLoad, setFinalLoad] = useState(false);
  const [refreshing, setRefreshing] = useState(false);
  const [loadingMore, setLoadingMore] = useState(false);

  const fetchVideosLiked = async (newOffset: number, isRefresh = false) => {
    try {
      const params = {
        filter: 'liked',
        offset: newOffset,
      ...
      const res: any = await ApiService.getVideosLiked(params);
      ...
      if (newOffset === 0 || isRefresh) {
        setListVideoLiked(items);
      } else {
        setListVideoLiked(prev => [...prev, ...items]);
      }

      setFinalLoad(items.length < PAGINATION.ITEMS_30 || (newOffset + items.length >= total));
      setOffset(newOffset);
    } catch (err) {
      console.log('fetchVideosLiked error:', err);
    } finally {
  ...
  const renderItem = ({ item }: any) => {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        onPress={() => {
          // If video detail exists, navigate
  ...
  return (
    <Wrapper>
      <CHeader
        ...
        isBorderBottom
        title={t('profile.liked', 'Đã thích')}
      ...
      {firstRender ? (
        ...
        <FlatList
          contentContainerStyle={listVideoLiked.length === 0 ? styles.emptyContainer : { paddingBottom: 24 }}
          data={listVideoLiked}
          numColumns={3}
          ...
          ListEmptyComponent={
            <CEmptyData title={t('profile.emptyLiked', 'Chưa có video đã thích')} />
          }
