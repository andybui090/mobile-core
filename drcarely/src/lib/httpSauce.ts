...
//   const instance: AxiosInstance = config.axiosInstance || axios.create(combinedConfig)
...
//       const problem = isError ? getProblemFromError(result) : getProblemFromStatus(status)
...
//       monitors.forEach(m => { try { m(our) } catch (e) {} })
//       return our
//     } catch (err) {
