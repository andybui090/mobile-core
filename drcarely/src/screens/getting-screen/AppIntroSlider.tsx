...
export default class AppIntroSlider<ItemT = any> extends React.Component<Props<ItemT>, State> {
  ...
  _renderButton = (name: string, label: string, onPress?: () => void, render?: () => React.ReactNode) => {
  ...
  _renderDefaultButton = (name: string, label: string) => {
    let content = <Text style={styles.buttonText} allowFontScaling={false}>{label}</Text>;
    if (this.props.bottomButton) {
      content = (
        <View style={[name === 'Skip' || name === 'Prev' ? styles.transparentBottomButton : styles.bottomButton]}>
          {content}
        </View>
      );
    }
  ...
  _renderOuterButton = (content: React.ReactNode, name: string, onPress?: (e: GestureResponderEvent) => void) => {
  ...
  _renderSkipButton = () =>
    ...
    this._renderButton(
      ...
      () => (this.props.onSkip ? this.props.onSkip() : this.goToSlide(this.props.data.length - 1)),
  ...
  render() {
    ...
    return (
      <View style={styles.flexOne}>
        <FlatList
          ref={ref => { this.flatList = ref as FlatList<ItemT> | undefined; }}
